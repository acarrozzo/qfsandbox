...
export const TeamBlade = forwardRef<HTMLDivElement, TeamBladeProps>(
  ({ team: initialTeam, userCount, ...props }, ref) => {
    const { onClick } = props
    const { t } = useTranslation(['teams'])
    const { hasPermission } = usePermissions()

    const { data: team } = trpcReactClient.teams.getTeam.useQuery(
      {
      ...
      },
      {
        ...
        initialData: {
          ...initialTeam,
          acl: {
            canUpdateTeam: hasPermission('customer-organization:administer'),
            canUpdateTeamProfile: hasPermission(
